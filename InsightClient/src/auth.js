 ...
 */
const LOGIN_URL = APIS.AUTH_LOGIN
 ...
 */
const REFRESH_TOKEN_URL = APIS.AUTH_LOGIN
...
export default {
  ...
  install (Vue, options) {
    Vue.http.interceptors.push((request, next) => {
      ...
      const hasAuthHeader = request.headers.has('access_token')
  ...
  login (creds, redirect) {
    ...
    return Vue.http.post(LOGIN_URL, params)
      .then((response) => {
        this._storeToken(response.body)
        if (redirect) {
          router.push({
            name: redirect
          })
  ...
  setAuthHeader (request) {
    request.headers.set('acess_token', store.state.auth.accessToken)
  ...
  _storeToken (data) {
    ...
    auth.isLoggedIn = data.AccessToken.IsAuthenticated
    auth.accessToken = data.AccessToken.Token
    auth.effectiveTime = data.AccessToken.EffectiveTime
    auth.expiresTime = data.AccessToken.ExpiresTime
